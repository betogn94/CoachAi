...
// Dashboard de marketing (pestaña Contenido): estado compartido por mes.
const isMes    = (s) => typeof s === 'string' && /^\d{4}-\d{2}$/.test(s);
const isCheckKey = (s) => typeof s === 'string' && /^w\d+-\d+-\d+$/.test(s);
const isKpiKey   = (s) => typeof s === 'string' && /^[a-z_]{1,30}$/.test(s);
...
export default withAuth(async (req, res, session) => {
  ...
  // ---------- DASHBOARD DE MARKETING (pestaña Contenido) ----------
  // El plan del mes vive en el front (Fase 1). Acá guardamos SOLO el estado
  // compartido por mes: los checks del calendario + los KPIs. Lo ve/edita todo el
  // equipo. Merge read-modify-write (checks/KPIs son de todos).
  if (req.query.mkt) {
    const mes = String(req.query.mes || '').slice(0, 7);
    if (!isMes(mes)) return badRequest(res, 'mes (YYYY-MM) requerido');

    if (method === 'GET') {
      const rows = await sb(`/team_marketing_estado?mes=eq.${mes}&select=mes,checks,kpis`);
      const estado = (rows && rows[0]) ? rows[0] : { mes, checks: {}, kpis: {} };
      return res.status(200).json({ ok: true, estado, me });
    }
    ...
    if (method === 'PATCH') {
      ...
      let checks = (cur && cur.checks && typeof cur.checks === 'object') ? cur.checks : {};
      let kpis   = (cur && cur.kpis   && typeof cur.kpis   === 'object') ? cur.kpis   : {};

      if (mb.resetChecks === true) checks = {};
      if (isCheckKey(mb.checkKey)) {
        if (mb.checkVal) checks[mb.checkKey] = true; else delete checks[mb.checkKey];
      }
      if (isKpiKey(mb.kpiKey)) {
        const v = String(mb.kpiVal == null ? '' : mb.kpiVal).slice(0, 40);
        if (v === '') delete kpis[mb.kpiKey]; else kpis[mb.kpiKey] = v;
      }

      const body = { mes, checks, kpis, updated_at: new Date().toISOString() };
      if (cur) {
        await sb(`/team_marketing_estado?mes=eq.${mes}`, { method: 'PATCH', body, prefer: 'return=minimal' });
      } else {
        body.created_by = me;
        await sb('/team_marketing_estado?on_conflict=mes', { method: 'POST', body, prefer: 'resolution=merge-duplicates,return=minimal' });
      }
      return res.status(200).json({ ok: true });
    }
